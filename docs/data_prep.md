...
- `path` — HuggingFace dataset identifier, e.g. `"stanfordnlp/imdb"` or `"HuggingFaceH4/ultrachat_200k"`.
...
- `data_dir` — Subdirectory of the dataset repository to load from (e.g. `"en"` for the English subset of `allenai/c4`), forwarded to `datasets.load_dataset(data_dir=...)`. `None` (default) omits the kwarg.

### Accepted path forms

`HuggingFaceDatasetLoader` accepts HuggingFace Hub identifiers: `"namespace/name"` pairs (e.g. `"stanfordnlp/imdb"`), bare legacy dataset names (e.g. `"squad"`), the explicit `"hf://datasets/..."` URL form, and `"buckets/..."` storage bucket paths.
...
loader.load("HuggingFaceH4/ultrachat_200k", split="train_sft")
...
loader.save("s3://my-bucket/data/ultrachat_sft.jsonl")
```
...
loader.filter(
    ...
    text_field="text",
    output_path="s3://my-bucket/filtered/ultrachat_dedup/",
)
loader.save("s3://my-bucket/data/ultrachat_dedup.jsonl")
```
