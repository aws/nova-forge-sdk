...
class TestValidator(unittest.TestCase):
    ...
    @patch("amzn_nova_forge_sdk.validation.validator.get_cluster_instance_info")
    @patch("amzn_nova_forge_sdk.validation.validator.boto3.client")
    @patch("sagemaker.get_execution_role")
    @patch(
        "amzn_nova_forge_sdk.manager.runtime_manager.SMHPRuntimeManager.required_calling_role_permissions"
    )
    def test_validate_smhp_infrastructure_success(
        ...
        try:
            with patch(
                "amzn_nova_forge_sdk.manager.runtime_manager.SMHPRuntimeManager.setup"
            ):
    ...
    def test_validate_iam_permissions_directly(self, mock_boto3_client):
        ...
        with patch("sagemaker.get_execution_role") as mock_get_role:
            with patch(
                "amzn_nova_forge_sdk.manager.runtime_manager.SMTJRuntimeManager.required_calling_role_permissions",
                return_value=[],
            ):
                with patch(
                    "amzn_nova_forge_sdk.manager.runtime_manager.SMTJRuntimeManager.setup"
                ):
