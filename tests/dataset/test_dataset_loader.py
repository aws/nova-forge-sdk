...
class TestDatasetLoader(unittest.TestCase):
    ...
    """
        ...
        self.assertEqual(
            list(dataset_loader.dataset())[0], {"id": "1", "name": "Alice"}
        )
        self.assertEqual(list(dataset_loader.dataset())[1], {"id": "2", "name": "Bob"})
        self.assertEqual(
            list(dataset_loader.dataset())[2], {"id": "3", "name": "Charlie"}
        )
    ...
    def test_openai_format_to_converse_format_nova_pro(self):
        ...
        dataset_loader.transform(
            training_method=TrainingMethod.SFT_FULL, model=Model.NOVA_PRO
        )
    ...
    def test_openai_format_to_converse_format_nova_lite_2(self):
        ...
        self.assertEqual(
            list(dataset_loader.dataset())[0]["messages"][0]["role"], "user"
        )
        self.assertEqual(
    ...
    def test_save_data_empty_dataset_warning(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "amzn_nova_forge.dataset.operations.save_operation.logger"
            ) as mock_logger:
                dataset_loader.save(str(save_path))
...
"""
    ...
    def test_cpt_with_newlines_and_tabs(self):
        ...
        self.assertEqual(
            list(dataset_loader.dataset())[0]["text"], text_with_formatting
        )
