...
class TestJobCacheContext(unittest.TestCase):
    def test_default_config(self):
        ...
        self.assertIsInstance(ctx.job_caching_config, JobCachingConfig)
    ...
    def test_custom_config_preserved(self):
        custom = JobCachingConfig(include_core=False, allowed_statuses=[JobStatus.COMPLETED])
...
class TestMatchesJobCacheCriteria(unittest.TestCase):
    def test_exact_match(self):
        ...
        config = JobCachingConfig(include_core=True, include_recipe=True, include_infra=False)
        self.assertTrue(matches_job_cache_criteria(config, h, h))

    def test_mismatch(self):
        h1 = "model:abc,method:def"
        h2 = "model:xyz,method:def"
        config = JobCachingConfig(include_core=True)
    ...
    def test_exclude_params(self):
        ...
        config = JobCachingConfig(include_core=True, exclude_params=["custom"])
    ...
    def test_include_infra(self):
        ...
        config = JobCachingConfig(include_core=True, include_infra=True)
        self.assertFalse(matches_job_cache_criteria(config, h1, h2))
...
class TestBuildCacheContext(unittest.TestCase):
    ...
    def test_job_caching_config_forwarded(self):
        custom_config = JobCachingConfig(
            include_core=True,
...
class TestLoadExistingResultValidation(unittest.TestCase):
    """Tests that Pydantic validates allowed_statuses at construction time."""

    def test_invalid_allowed_statuses_rejected_by_pydantic(self):
