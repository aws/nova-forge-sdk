...
class TestDatasetLoader(unittest.TestCase):
    ...
    def test_end_to_end_batch_all_with_system(self):
        """Test batch processing where all samples have a system message."""
        import json
        import tempfile
        ...
        try:
            ...
            records = list(dataset_loader.dataset())
            self.assertEqual(len(records), 2)
            # All records should have system key
            for rec in records:
    ...
    def test_end_to_end_batch_none_with_system(self):
        """Test batch processing where no samples have a system message."""
        import json
        import tempfile
        ...
        try:
            ...
            records = list(dataset_loader.dataset())
            self.assertEqual(len(records), 2)
            # No records should have system key
            for rec in records:
                self.assertNotIn("system", rec)
            # Check multi-turn conversation structure
            self.assertEqual(len(records[1]["messages"]), 4)
        finally:
            import os

            os.unlink(temp_path)

    def test_end_to_end_batch_mixed_system_raises(self):
        """Test that mixed system message presence across samples raises a consistency error."""
        import json
        import tempfile
        ...
        try:
            ...
            self.assertIn("Dataset consistency error", str(ctx.exception))
