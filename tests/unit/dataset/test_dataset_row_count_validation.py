...
class TestBaseDatasetValidatorRowCounts(unittest.TestCase):
    ...
    # -- valid_max_limit checks --

    def test_valid_max_limit_exceeded_raises(self):
        """Exceeding valid_max_limit for BEDROCK/SFT/NOVA_LITE_2 should NOT raise in
    ...
    def test_valid_max_limit_at_limit_passes(self):
    ...
    def test_valid_max_limit_not_applied_to_smtj(self):
        """SMTJ platform should not enforce the BEDROCK valid_max_limit limit."""
        samples = [_make_sft_sample(f"q{i}") for i in range(20001)]

        # Should not raise — valid_max_limit check only applies to BEDROCK
        _make_loader(samples).validate(
            training_method=TrainingMethod.SFT_LORA,
            model=Model.NOVA_LITE_2,
    ...
    def test_valid_max_limit_applied_to_nova_lite_on_bedrock(self):
    ...
    # -- hard valid_min_limit checks (fixed limits from DATASET_CHECKS config) --

    def test_hard_valid_min_limit_below_minimum_raises(self):
    ...
    def test_smtj_skips_recipe_dependent_min_check(self):
        """SMTJ valid_min_limit_recipe_field check is skipped in loader.validate (no recipe).
    ...
    def test_cpt_validator_forwards_row_count_kwargs(self):
        ...
        # CPT has no BEDROCK valid_max_limit check in current config, so this should pass
        # (CPT is not in the VALIDATION_CHECKS for valid_max_limit)
        validator.validate(
            iter(samples),
            Model.NOVA_LITE_2,
            platform=Platform.BEDROCK,
...
class TestValidatorDatasetRowCounts(unittest.TestCase):
    ...
    def test_valid_max_limit_exceeded_appends_error(self, mock_count):
        errors = []
    ...
    def test_valid_max_limit_at_limit_no_error(self, mock_count):
        errors = []
        Validator._validate_dataset_row_counts(
            platform=Platform.BEDROCK,
            method=TrainingMethod.SFT_LORA,
    ...
    def test_valid_min_limit_below_recipe_field_appends_error(self, mock_count):
        errors = []
        Validator._validate_dataset_row_counts(
            platform=Platform.SMTJ,
    ...
    def test_valid_min_limit_at_recipe_field_no_error(self, mock_count):
        errors = []
        Validator._validate_dataset_row_counts(
            platform=Platform.SMTJ,
            method=TrainingMethod.SFT_FULL,
    ...
    def test_missing_recipe_field_skips_min_check(self, mock_count):
        """If recipe doesn't contain the valid_min_limit_recipe_field, skip that check."""
        errors = []
        Validator._validate_dataset_row_counts(
            platform=Platform.SMTJ,
            method=TrainingMethod.SFT_LORA,
    ...
    def test_bedrock_sft_full_also_checked(self, mock_count):
        """SFT_FULL should also be subject to the BEDROCK valid_max_limit check."""
        errors = []
        Validator._validate_dataset_row_counts(
            platform=Platform.BEDROCK,
            method=TrainingMethod.SFT_FULL,
...
class TestValidationChecksConfig(unittest.TestCase):
    ...
    def test_all_row_count_entries_have_required_keys(self):
        ...
        for check in DATASET_CHECKS:
            ...
            self.assertIn("applicable_training_methods", check)
            self.assertIn("applicable_platforms", check)
            self.assertIn("applicable_models", check)
            self.assertFalse(
                check.get("filterable", False),
                f"Row-count check '{check['name']}' should not be filterable",
            )
            self.assertTrue(
                "valid_max_limit" in check
                or "valid_min_limit" in check
                or "valid_min_limit_recipe_field" in check,
                f"Check '{check['name']}' must have valid_max_limit, valid_min_limit, or valid_min_limit_recipe_field",
            )

    def test_bedrock_sft_nova_lite_2_valid_max_limit_is_20000(self):
        from amzn_nova_forge.dataset.configs.dataset_checks_config import (
            DATASET_CHECKS,
        )
        ...
        self.assertEqual(entry["valid_max_limit"], 20000)
        self.assertEqual(entry["valid_min_limit"], 200)
        self.assertIn(Platform.BEDROCK, entry["applicable_platforms"])
        self.assertIn(TrainingMethod.SFT_LORA, entry["applicable_training_methods"])
        self.assertIn(TrainingMethod.SFT_FULL, entry["applicable_training_methods"])
        self.assertIn(Model.NOVA_LITE_2, entry["applicable_models"])

    def test_valid_min_limit_sft_nova_lite_2_uses_global_batch_size(self):
        from amzn_nova_forge.dataset.configs.dataset_checks_config import (
            DATASET_CHECKS,
        )

        entry = next(c for c in DATASET_CHECKS if c["name"] == "min_dataset_rows_sft_nova_lite_2")
        self.assertEqual(entry["valid_min_limit_recipe_field"], "global_batch_size")
        self.assertIn(Platform.SMTJ, entry["applicable_platforms"])
        self.assertIn(Platform.SMHP, entry["applicable_platforms"])
