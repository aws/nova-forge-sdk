...
class InferenceResult(BaseJobResult, ABC):
    ...
    def _reformat_inference_results(line) -> Dict:
        ...
        system_match = re.search(r"'role':\s*'system',\s*'content':\s*'([^']+)'", entry["prompt"])
    ...
    def _download_inference_results(self) -> str:
        ...
        try:
            ...
            logger.info(f"Successfully downloaded inference results to {self._cached_results_dir}")
    ...
    def get(self, s3_path=None) -> Dict:
        ...
        if job_status == JobStatus.COMPLETED:
                ...
                with open(results_file[0], "r") as f:
                    ...
                    if s3_path:
                        try:
                            ...
                            logger.error(f"Error saving inference results to {s3_path}: {e}")
                            raise
                    return {"inference_results": inference_dataset}
            else:
                print(f"No inference output jsonl file found for job {self.job_id}")
                logger.info(f"No inference output jsonl file found for job {self.job_id}")
                return {}
            ...
            print(f"Cannot show inference result. Job '{self.job_id}' in {raw_status} status.")
            logger.info(
