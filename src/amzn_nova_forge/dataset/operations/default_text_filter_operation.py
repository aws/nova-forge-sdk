...
"""Default text filter operation for data preparation pipelines.
...
Usage standalone::
    ...
    from amzn_nova_forge.dataset.operations.default_text_filter_operation import DefaultTextFilterOperation
...
_PIPELINE_ID = "default_text_filter"


class DefaultTextFilterOperation(NovaForgeFilterOperationBase):
    """Default text filter pipeline operation (composite).

    A composite filter that currently removes records with excessive URL
    content.  Additional sub-filters (e.g. language detection, PII
    scrubbing, length checks) will be added over time — callers should
    not assume a fixed set of checks.

    Internally delegates to the ``default_text_filter`` ForgeWorkflows
    pipeline.
    ...
    _FILTER_NAME = "Default Text Filter"
